...
export const Mixins = {
  ...
  init(options) {
    ...
    const rawValue = options.value.toString().split('');
    ...
    this.time = setTimeout(() => {
      ...
      const input = key => {
        ...
        if (formatFn(newValue)) {
          ...
          } else if (newValue.length > maxlength || type === 'tel' &&
              !RTel.test(newValue)) { return; }
  ...
  openKeyboard() {
    ...
    elContainer.style.cssText = 'z-index: 999; position:absolute; bottom:0; left:0; width:100%; height:33%;';
    //elShadow.style.cssText = 'height:100%;';
    elKeyboard.style.cssText = 'position:absolute; top:0; right: 0; bottom: 0; left:0; ' +
        'transform: translateY(100%); transition: transform 0.35s ease 0s; box-shadow: 1px -1px 3px 0px #6e717570;';
